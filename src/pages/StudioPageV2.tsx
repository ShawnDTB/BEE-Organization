...
const textColors: string[] = ['#f4f5f5', '#111417', '#9bb7c8', '#d1b46a', '#8e5c5c', '#567a63', '#725f8e'];
...
function GarmentSvg({ garment, color, view }: { garment: GarmentType; color: string; view: GarmentView }) {
  ...
  if (garment === 'hoodie') {
    return (
      <svg viewBox="0 0 500 610" className="studio-garment-svg" role="img" aria-label={`${view} view of heavyweight hoodie`}>
        <defs><linearGradient id="studioHoodieShade" x1="0" x2="1"><stop offset="0" stopColor={color} /><stop offset="0.5" stopColor={color} stopOpacity="0.86" /><stop offset="1" stopColor={color} /></linearGradient></defs>
        <path d="M166 112 L205 88 Q250 58 295 88 L334 112 L421 190 L371 268 L332 236 L327 548 L173 548 L168 236 L129 268 L79 190 Z" fill="url(#studioHoodieShade)" stroke={stroke} strokeWidth="3" />
  ...
  if (garment === 'polo') {
    return (
      <svg viewBox="0 0 500 610" className="studio-garment-svg" role="img" aria-label={`${view} view of performance polo`}>
        <defs><linearGradient id="studioPoloShade" x1="0" x2="1"><stop offset="0" stopColor={color} /><stop offset="0.5" stopColor={color} stopOpacity="0.88" /><stop offset="1" stopColor={color} /></linearGradient></defs>
        <path d="M165 112 L210 92 Q250 78 290 92 L335 112 L420 180 L378 260 L329 228 L325 548 L175 548 L171 228 L122 260 L80 180 Z" fill="url(#studioPoloShade)" stroke={stroke} strokeWidth="3" />
  ...
  return (
    <svg viewBox="0 0 500 610" className="studio-garment-svg" role="img" aria-label={`${view} view of premium tee`}>
      <defs><linearGradient id="studioTeeShade" x1="0" x2="1"><stop offset="0" stopColor={color} /><stop offset="0.5" stopColor={color} stopOpacity="0.88" /><stop offset="1" stopColor={color} /></linearGradient></defs>
      <path d="M168 108 L211 88 Q250 104 289 88 L332 108 L425 176 L381 265 L329 233 L325 548 L175 548 L171 233 L119 265 L75 176 Z" fill="url(#studioTeeShade)" stroke={stroke} strokeWidth="3" />
...
export function StudioPage() {
  const startingGarment = initialGarment();
  const [garment, setGarment] = useState<GarmentType>(startingGarment);
  const [color, setColor] = useState<string>(garmentColors[0]?.value ?? '#15191d');
  const [view, setView] = useState<GarmentView>('front');
  const [decoration, setDecoration] = useState<DecorationType>(startingGarment === 'polo' ? 'embroidery' : 'graphic');
  const [text, setText] = useState<string>('YOUR DESIGN');
  const [textColor, setTextColor] = useState<string>(textColors[0] ?? '#f4f5f5');
  ...
  const [y, setY] = useState<number>(45);
  const [scale, setScale] = useState<number>(100);
  const [size, setSize] = useState<string>(garmentMeta[startingGarment].sizes[2] ?? garmentMeta[startingGarment].sizes[0] ?? 'M');
  ...
  function saveDraft() {
    ...
    const drafts = readArray<StudioDraft>(DRAFT_KEY);
  ...
  function addToProjectBag() {
    ...
    const bag = readArray<ProjectBagItem>(CART_KEY);
    ...
    <section className="studio-page">
      <header className="studio-heading">
        <div>
          ...
          <h1>Design the piece before production ever touches it.</h1>
        </div>
        <p>Choose a garment, change its color, add text or artwork, select embroidery or graphic decoration, and position the design. This is a planning visual — final supplier templates and production proofs still control the real order.</p>
      </header>
        ...
        <div className="studio-viewport-column">
          ...
          <div className="studio-viewport">
            ...
            {artworkVisibleOnCurrentView ? (
              <div className={`studio-art-layer studio-art-layer--${decoration}`} style={{ left: `${x}%`, top: `${y}%`, transform: `translate(-50%, -50%) scale(${scale / 100})` }}>
                {artworkData && <img src={artworkData} alt="Uploaded artwork preview" />}
                {text.trim() && <span style={{ color: textColor }}>{text}</span>}
              </div>
            ) : (
              <div className="studio-surface-hint">Design is currently placed on the {currentPlacement.view}.</div>
            )}
            ...
            <div className="studio-garment-options">
              {(Object.keys(garmentMeta) as GarmentType[]).map((option) => (
                <button key={option} type="button" className={garment === option ? 'is-active' : undefined} onClick={() => changeGarment(option)}>
                  <img src={garmentMeta[option].image} alt="" /><span>{garmentMeta[option].label}</span>
                </button>
              ))}
            </div>
      ...
      <footer className="studio-footnote">
        ...
        <div><span>Future 3D layer</span><strong>Architecture ready</strong><p>The draft already separates garment, surface, decoration, artwork, position, scale, size, and quantity so real 3D models can replace the visual renderer later.</p></div>
      </footer>
    </section>
  );
}
