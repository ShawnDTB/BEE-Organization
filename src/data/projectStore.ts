...
export function readDrafts(): StudioDraft[] {
  return readArray<StudioDraft>(DRAFT_KEY).map((draft) => ({
    ...draft,
...
export function saveStudioDraft(draft: StudioDraft) {
  ...
  const next = index >= 0 ? drafts.map((item) => item.id === draft.id ? draft : item) : [...drafts, draft];
...
export function duplicateStudioDraft(id: string) {
  ...
  const copy: StudioDraft = { ...source, id: `studio-${Date.now()}`, name: `${source.name} copy`, createdAt: now, updatedAt: now };
...
export function renameStudioDraft(id: string, name: string) {
  ...
  if (!draft) return;
  saveStudioDraft({ ...draft, name: name.trim() || draft.name, updatedAt: new Date().toISOString() });
}

export function deleteStudioDraft(id: string) {
  write(DRAFT_KEY, readDrafts().filter((draft) => draft.id !== id));
  const bag = readBag().filter((item) => item.draftId !== id && item.id !== id);
  saveBag(bag);
  window.dispatchEvent(new Event(PROJECT_EVENT));
}

export function readBag(): ProjectBagItem[] {
  return readArray<ProjectBagItem>(CART_KEY).map((item) => ({ ...item, draftId: item.draftId || (item.id.startsWith('studio-') ? item.id : undefined) }));
}
...
export function upsertBagItem(item: ProjectBagItem) {
  const items = readBag();
  const index = items.findIndex((existing) => existing.id === item.id || (item.draftId && existing.draftId === item.draftId));
  saveBag(index >= 0 ? items.map((existing, i) => i === index ? item : existing) : [...items, item]);
}

export function readProjectDraft(): ProjectIntake {
  return { ...emptyProjectIntake, ...parseJson<Partial<ProjectIntake>>(window.localStorage.getItem(PROJECT_DRAFT_KEY), {}) };
}
...
function nextReference(projects: BeeProject[]) {
  const highest = projects.reduce((max, project) => {
    const match = project.reference.match(/BEE-(\d+)/i);
...
export function createProject(intake: ProjectIntake, items = readBag()) {
  ...
  const now = new Date().toISOString();
  const reference = nextReference(projects);
