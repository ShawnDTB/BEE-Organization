...
import { MAX_PREVIEW_BYTES } from "../../shared/request";
import { safeQuantity } from "../data/projectStore";
...
const garmentMeta: Record<
  GarmentType,
  { label: string; productSlug: string; sizes: string[]; image: string }
> = {
  tee: {
    label: "Premium Tee",
  ...
  hoodie: {
    ...
    productSlug: "heavyweight-creator-hoodie",
    sizes: ["S", "M", "L", "XL", "2XL", "3XL"],
    image: "/store/hoodie.svg",
  },
  polo: {
    label: "Performance Polo",
...
const placementPresets: Record<
  PlacementPreset,
  { label: string; x: number; y: number; scale: number; view: GarmentView }
> = {
...
export function GarmentSvg({
  ...
  const stroke =
    color === "#e8e8e4" || color === "#d7d0c2" ? "#8b9295" : "#626d75";
  if (garment === "hoodie") {
    return (
      <svg
        viewBox="0 0 500 610"
        className="studio-garment-svg"
        role="img"
        aria-label={`${view} view of heavyweight hoodie`}
      >
        <defs>
          <linearGradient id="studioHoodieShade" x1="0" x2="1">
            <stop offset="0" stopColor={color} />
            <stop offset=".5" stopColor={color} stopOpacity=".86" />
            <stop offset="1" stopColor={color} />
          </linearGradient>
        </defs>
        <path
          d="M166 112 L205 88 Q250 58 295 88 L334 112 L421 190 L371 268 L332 236 L327 548 L173 548 L168 236 L129 268 L79 190 Z"
          fill="url(#studioHoodieShade)"
          stroke={stroke}
          strokeWidth="3"
        />
        <path
          d="M204 92 Q250 32 296 92 Q286 150 250 158 Q214 150 204 92Z"
          fill={color}
          stroke={stroke}
          strokeWidth="3"
        />
        {view === "front" && (
          <>
            <path d="M220 132 L215 224" stroke={stroke} strokeWidth="3" />
            <path d="M280 132 L285 224" stroke={stroke} strokeWidth="3" />
            <path
              d="M192 425 L308 425 L332 493 L168 493 Z"
              fill="none"
              stroke={stroke}
              strokeWidth="3"
            />
  ...
  if (garment === "polo") {
    return (
      <svg
        ...
        className="studio-garment-svg"
        role="img"
        aria-label={`${view} view of performance polo`}
      >
        <defs>
          <linearGradient id="studioPoloShade" x1="0" x2="1">
            <stop offset="0" stopColor={color} />
            <stop offset=".5" stopColor={color} stopOpacity=".88" />
            <stop offset="1" stopColor={color} />
          </linearGradient>
        </defs>
        <path
          d="M165 112 L210 92 Q250 78 290 92 L335 112 L420 180 L378 260 L329 228 L325 548 L175 548 L171 228 L122 260 L80 180 Z"
          fill="url(#studioPoloShade)"
          stroke={stroke}
          strokeWidth="3"
        />
        {view === "front" && (
          <>
            <path
              d="M210 92 L250 137 L290 92 L309 127 L273 155 L250 138 L227 155 L191 127 Z"
              fill="none"
              stroke={stroke}
              strokeWidth="3"
            />
            <path d="M250 138 L250 223" stroke={stroke} strokeWidth="3" />
            <circle cx="250" cy="171" r="3" fill={stroke} />
            <circle cx="250" cy="194" r="3" fill={stroke} />
          </>
        )}
      </svg>
    );
  ...
  return (
    <svg
      ...
      className="studio-garment-svg"
      role="img"
      aria-label={`${view} view of premium tee`}
    >
      <defs>
        <linearGradient id="studioTeeShade" x1="0" x2="1">
          <stop offset="0" stopColor={color} />
          <stop offset=".5" stopColor={color} stopOpacity=".88" />
          <stop offset="1" stopColor={color} />
        </linearGradient>
      ...
      <path
        d="M168 108 L211 88 Q250 104 289 88 L332 108 L425 176 L381 265 L329 233 L325 548 L175 548 L171 233 L119 265 L75 176 Z"
        fill="url(#studioTeeShade)"
        stroke={stroke}
        strokeWidth="3"
      />
      <path
        d="M211 88 Q250 128 289 88 Q283 151 250 157 Q217 151 211 88Z"
        fill={color}
        stroke={stroke}
        strokeWidth="3"
      />
...
export function StudioPageV3() {
  ...
  const [status, setStatus] = useState<string | null>(
    getDraft(source.id) ? "Draft reopened from this browser." : null,
  );
  ...
  const designLabel = useMemo(() => {
    const parts = [
      decoration === "embroidery" ? "Embroidery" : "Graphic decoration",
  ...
  function handleArtwork(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!["image/png", "image/jpeg", "image/webp"].includes(file.type)) {
      setStatus("Choose an image file for the artwork preview.");
      return;
    }
    if (file.size > Math.floor(MAX_PREVIEW_BYTES * 0.74)) {
      setStatus(
        "Use an image under 250 KB for the preview. Keep your original file for production review.",
      );
      return;
    }
    ...
    reader.onerror = () =>
      setStatus(
        "This image could not be opened. Try another PNG, JPEG or WebP.",
      );
    reader.onload = () => {
  ...
  function saveDraft() {
    ...
    setStatus(
      "Draft saved. You can reopen this exact design from the customer workspace.",
    );
  }
  ...
  function addToProjectBag() {
    try {
      ...
      upsertBagItem({
        ...
        color:
          garmentColors.find((option) => option.value === color)?.label ??
          color,
        size,
        quantity: safeQuantity(quantity),
      ...
      setStatus(
        readBag().some((item) => item.draftId === draft.id)
          ? "Design saved and synced with your project bag."
          : "Design saved and added to your project bag.",
      );
    } catch (e) {
      setStatus((e as Error).message);
    }
  ...
  return (
    <section className="studio-page studio-page--v3">
      <header className="studio-v3-intro">
        ...
        <p>
          Choose a garment, add text or artwork, set the decoration and
          placement, then keep the exact draft with your project.
        </p>
      </header>
      ...
      <div className="studio-shell">
        <div className="studio-viewport-column">
          <div className="studio-toolbar">
            ...
            >
              <button
                type="button"
                className={view === "front" ? "is-active" : undefined}
              ...
              <button
                type="button"
                className={view === "back" ? "is-active" : undefined}
          ...
          <div className="studio-viewport">
            ...
            {artworkVisibleOnCurrentView ? (
              ...
              >
                {artworkData && (
                  <img src={artworkData} alt="Uploaded artwork preview" />
                )}
                {text.trim() && (
              ...
              <div className="studio-surface-hint">
                Design is currently placed on the {currentPlacement.view}.
              </div>
            )}
            <div className="studio-safe-area" aria-hidden="true">
              <span>Suggested decoration area</span>
            </div>
          </div>
          <div className="studio-mobile-status">
            <span>{currentPlacement.label}</span>
            <b>{safeQuantity(quantity)} pieces</b>
            <span>{size}</span>
          </div>
        </div>
        ...
        <aside
          ...
          aria-label="Garment designer controls"
        >
          <div className="studio-controls__section">
            <div className="studio-controls__title">
              <span>01</span>
            ...
            <div className="studio-garment-options">
              {(Object.keys(garmentMeta) as GarmentType[]).map((option) => (
                <button
                  key={option}
                  type="button"
                  className={garment === option ? "is-active" : undefined}
                  aria-pressed={garment === option}
                  onClick={() => changeGarment(option)}
                >
                  <img src={garmentMeta[option].image} alt="" />
            ...
            <label className="studio-field">
              ...
              <div className="studio-swatches">
                {garmentColors.map((option) => (
                  <button
                    key={option.value}
                    type="button"
                    className={color === option.value ? "is-active" : undefined}
            ...
            <div className="studio-two-up">
              <label className="studio-field">
                ...
                >
                  {currentGarment.sizes.map((option) => (
          ...
          <div className="studio-controls__section">
            ...
            <div className="studio-segmented">
              <button
                type="button"
                className={
                  decoration === "embroidery" ? "is-active" : undefined
                }
                aria-pressed={decoration === "embroidery"}
                onClick={() => setDecoration("embroidery")}
              >
                Embroidery
              </button>
              <button
                type="button"
                className={decoration === "graphic" ? "is-active" : undefined}
                aria-pressed={decoration === "graphic"}
                onClick={() => setDecoration("graphic")}
              >
                Graphic / print
              </button>
            </div>
            ...
            <label className="studio-field">
              ...
              <div className="studio-swatches studio-swatches--small">
                {textColors.map((option) => (
                  <button
                    key={option}
                    type="button"
                    className={textColor === option ? "is-active" : undefined}
            ...
            <label className="studio-upload">
              ...
              <b>
                {artworkData
                  ? "Artwork loaded"
                  : "PNG, JPG or WEBP · under 250 KB"}
              </b>
            </label>
            {artworkData && (
              <button
                className="studio-remove-art"
                type="button"
                onClick={() => setArtworkData(undefined)}
              >
          ...
          <div className="studio-controls__section">
            ...
            <label className="studio-field">
              ...
              <select
                value={placement}
                onChange={(event) =>
                  applyPlacement(event.target.value as PlacementPreset)
                }
              >
                {(Object.keys(placementPresets) as PlacementPreset[]).map(
                  (option) => (
                    <option key={option} value={option}>
                      {placementPresets[option].label}
                    </option>
                  ),
                )}
              </select>
          ...
          <div className="studio-controls__actions">
            <button className="button" type="button" onClick={addToProjectBag}>
              Save + add to project bag
            </button>
      ...
      <footer className="studio-v3-note">
        <div>
          ...
          <span>Garment shape, color, and placement are approximate.</span>
        </div>
        <div>
          <strong>Final proof required.</strong>
          <span>
            Artwork, decoration, availability, pricing, and placement are
